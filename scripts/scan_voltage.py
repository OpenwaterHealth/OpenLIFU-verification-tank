...
with LIFUInterface(ext_power_supply=True) as interface, Picoscope(resolution="15BIT") as scope, QPX600DP() as hv:
    ...
    for voltage in voltages:            
        for output in [1,2]:
            hv.set_voltage(output, voltage=voltage)
        for output in [1,2]:
            hv.wait_ready(output, target=voltage)
...
t = outputs[0]["time"]
outputs = np.array([output["A"] for output in outputs]).reshape([len(voltages), -1])
savedata = {'t':t, "outputs":outputs, "voltages":voltages}
